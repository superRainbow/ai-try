...
def create_collection(collection_name):
    fields = [
        ...
        # 這案例中使用 embedding 模型是 all-MiniLM-L6-v2，有 384 個維度，dim 就要使用 384
        FieldSchema(name='embedding', dtype=DataType.FLOAT_VECTOR,
                    description='Embedding vectors', dim=384)
    ]
    ...
    index_params = {
        'index_type': 'HNSW',
        'metric_type': 'COSINE',
        'params': {'efConstruction': 500, 'M': 1024}
    }
...
def insert(collection, text_array, embedding_array):
    # zip 對應的元素打包成一個個元組
    entities = [
        {"id": i, "lyric": sentence, "embedding": embedding}
...
def search(collection, query_embedding, k=1):
    search_params = {
        "metric_type": "COSINE"
    }
